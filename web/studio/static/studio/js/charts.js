window.PredictaCharts = (() => {
  ...
  const palette=['#7c869e','#39e67a','#ff9640','#7c7ff5','#f5d547'];
  ...
  function multiLine(id,labels,series,o={}){
    ...
    for(let i=0;i<=4;i++){const yy=p.t+i*(H-p.t-p.b)/4;svg.appendChild(el('line',{x1:p.l,x2:W-p.r,y1:yy,y2:yy,style:'stroke:var(--line)','stroke-width':1}));const t=el('text',{x:p.l-8,y:yy+4,'text-anchor':'end',style:'fill:var(--muted)','font-size':11});t.textContent=fmt(ymax-i*(ymax-ymin)/4,o);svg.appendChild(t)}
    const tickStep=Math.max(1,Math.ceil(labels.length/8)); labels.forEach((lab,i)=>{if(i%tickStep&&i!==labels.length-1)return;const t=el('text',{x:x(i),y:H-22,'text-anchor':'middle',style:'fill:var(--muted)','font-size':10});t.textContent=String(lab).replace('T',' ').slice(0,13);svg.appendChild(t)});
    ...
    let lx=p.l;series.forEach((s,si)=>{svg.appendChild(el('line',{x1:lx,x2:lx+18,y1:14,y2:14,stroke:palette[si%palette.length],'stroke-width':3}));const t=el('text',{x:lx+24,y:18,style:'fill:var(--ink)','font-size':11});t.textContent=s.name;svg.appendChild(t);lx+=24+(s.name.length*8)+28;});
  }
